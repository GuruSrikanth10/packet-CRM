...
on any other tier. Only the `complex` tier is consumed by the graph today; the
`simple` tier is constructed but unused. `.env`, `.env.example`, and
`llm_utils.py` all use this same `_COMPLEX`/`_SIMPLE` env var suffix
vocabulary; `config_validator.py` checks whichever key the *selected*
provider (`USE_HF` / `MOCK_LLM_WITH_MISTRAL` / default OpenAI-compatible)
actually reads, not a hardcoded `OPENAI_API_KEY`.
...
The Phase 0 (correctness-breaking, P0) and Phase 1 (reliability/operability, P1)
items from the remediation plan (`REMEDIATION_PLAN.md`) have been implemented and
are covered by `tests/test_phase0_fixes.py` and `tests/test_phase1_fixes.py`. What
remains:
...
| 3 | Reviewer agent cost/latency (Phase 2, 2.1/2.2) | The Reviewer's React agent is still rebuilt on every review (not cached like Investigator/Synthesis), and the constructed-but-unused `simple` LLM tier is still not wired to anything. Tracked as Phase 2 optimizations, not correctness bugs. |
| 4 | Full Kafka payload resent on every retry (Phase 2, 2.3) | The Investigator prompt still receives the entire Kafka payload via `json.dumps(payload)` on every retry loop iteration, rather than a projected subset plus just the delta. Cost optimization, not a correctness issue. |
| 5 | `print()` vs structured logging (Phase 2, 2.8) | `agent_orchestrator.py`, `tool_registry.py`, and `log_pipeline/` still use bare `print()` rather than the `structlog` logger used in `routes.py`, so those lines aren't correlated by `event_id` or machine-parseable. |

See `REMEDIATION_PLAN.md`'s Phase 2 section for the full list of remaining
optimizations (none of which change observable correctness).
